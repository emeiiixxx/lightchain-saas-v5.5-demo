...
export function Button({ variant = 'ghost', size = 's', icon, children, className = '', title, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'ghost' | 'primary' | 'outline' | 'secondary' | 'tonal' | 'danger'; size?: 's' | 'm' | 'l'; icon?: string }) {
...
export function Dialog({ title, children, onClose, phase = 'enter', className = '', closeLabel = '关闭', closeIcon = 'close' }: { title: ReactNode; children: ReactNode; onClose: () => void; phase?: 'enter' | 'exit'; className?: string; closeLabel?: string; closeIcon?: string }) {
  const { t } = useLocale();
